...
class RencanaKerjaHRD extends BaseController
{
    ...
    public function inputhrd()
    {
        return view('dashboard/hrd/input_rencana_kerja');
    }

    // Proses input rencana kerja
    public function saveRencanaKerjahrd()
    {
        ...
        return redirect()->to('/rencana-kerja-hrd/input')->with('message', 'Rencana Kerja berhasil ditambahkan');
    }

    // Halaman untuk melihat riwayat rencana kerja
    public function riwayat()
{
    // Ambil user_id dan divisi_id dari session
    $user_id = session()->get('user_id');  // Ambil user_id dari session
    $divisi_id = session()->get('divisi_id'); // Ambil divisi_id dari session

    // Pastikan user_id dan divisi_id ada di session
    if (!$divisi_id) {
    ...
    $rencana_kerja = $this->rencanaKerjaModel
        ...
        ->join('users AS u1', 'u1.id = rencana_kerja.manager_id', 'left') // Manager_id harus mengacu ke manager
        ->where('users.divisi_id', $divisi_id) // Filter berdasarkan divisi_id yang ada di session
        ->where('rencana_kerja.user_id', $user_id)
        ->orderBy('rencana_kerja.tanggal', 'DESC')// Pastikan manager tidak ikut dalam hasil
        ->findAll(); // Ambil semua data yang sesuai

    // Kirim data ke view
    return view('dashboard/hrd/riwayat_rencana_kerja', [
    ...
    public function edit($id)
    {
        ...
        if (!$rencana) {
            return redirect()->to('/rencana-kerja-hrd/riwayat')->with('error', 'Rencana kerja tidak ditemukan');
        }

        return view('hrd/edit_rencana_kerja', ['rencana' => $rencana]);
    }
    ...
    public function update($id)
    {
        ...
        return redirect()->to('/rencana-kerja-hrd/riwayat')->with('message', 'Rencana Kerja berhasil diperbarui');
    }
}
