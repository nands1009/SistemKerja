...
<head>
    ...
    <title>Riwayat Pengajuan Manajer</title>
    <style>
        /* Global Styles */
        body {
        ...
        <div class="container">
            ...
            <div class="table-container">
                ...
                <table id="penghargaanTable">
                    ...
                    <tbody>
                        <?php $no = 1;
                        foreach ($riwayat_penghargaan as $penghargaan): ?>
                            <tr>
                                <td><?= $no++; ?></td>
                                ...
                                <td><?= $penghargaan['updated_at']; ?></td>
                                <td><?= $penghargaan['status']; ?></td>
                                <td><?= $penghargaan['catatan_penolakan']; ?></td>
                                <td class="action-buttons">
                                    <!-- Button for PDF Upload for Penghargaan -->
                                    <?php if ($penghargaan['status'] == 'Approved DIREKSI' && empty($penghargaan['file_name'])): ?>
                                        <form action="/pdf/upload/penghargaan/<?= $penghargaan['id']  ?>" method="post" enctype="multipart/form-data">
                                            <?= csrf_field(); ?>
                                            <input type="file" name="pdf_file" accept="application/pdf">
                                            <button type="submit">Upload PDF</button>
                                        </form>
                                    <?php elseif (!empty($penghargaan['file_name'])): ?>
                                        <p>PDF sudah diupload.</p> <!-- Kamu bisa mengganti pesan ini sesuai kebutuhan -->
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                ...
                <tbody>
                    ...
                    foreach ($riwayat_sp as $sp): ?>
                        <tr>
                            ...
                            <td><?= $sp['updated_at']; ?></td>
                            <td><?= $sp['status']; ?></td>
                            <td><?= $sp['catatan_penolakan']; ?></td>
                            <td class="action-buttons">
                                <!-- Form for Upload PDF for SP -->
                                <?php if ($sp['status'] == 'Approved DIREKSI' && empty($sp['file_name'])): ?>
                                    <form action="/pdf/upload/<?= $sp['id']  ?>" method="post" enctype="multipart/form-data">
                                        <?= csrf_field(); ?>
                                        <input type="file" name="pdf_file" accept="application/pdf">
                                        <button type="submit">Upload PDF</button>
                                    </form>
                                <?php elseif (!empty($sp['file_name'])): ?>
                                    <p>PDF sudah diupload.</p> <!-- Kamu bisa mengganti pesan ini sesuai kebutuhan -->
                                <?php endif; ?>
                            </td>
                        </tr>
